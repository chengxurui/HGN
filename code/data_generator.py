...
class input_data(object):
 ...
 # 直接导入自己的数据 可以不用这么处理
			...
			p_abstract_embed = np.zeros((self.args.P_n, self.args.in_f_d))   # in_f_d：input feature dimension  p_abstract_embed.shape:(21044, 128)
			p_a_e_f = open(self.args.data_path + "p_abstract_embed.txt", "r")  # 论文的摘要特征
			for line in islice(p_a_e_f, 1, None):
				values = line.split()
				index = int(values[0])
				embeds = np.asarray(values[1:], dtype='float32')
				p_abstract_embed[index] = embeds
			# print('pr:',p_abstract_embed)
			p_a_e_f.close()

			p_title_embed = np.zeros((self.args.P_n, self.args.in_f_d))
			p_t_e_f = open(self.args.data_path + "p_title_embed.txt", "r")  # 论文的标题特征
			...
			#  分别得到特征
			a_net_embed = np.zeros((self.args.A_n, self.args.in_f_d))  #存放的是作者携带的特征
			...
			for line in islice(net_e_f, 1, None):
				line = line.strip()  # 是删除字符串前导和尾随空格的字符串副本
				index = re.split(' ', line)[0]  # 0.054210365 v17 v14 v8 等下标
				# print(index)
			...
			# print('df',a_neigh_list_train)

			het_neigh_train_f.close()
			# print('c',a_neigh_list_train[0][1])
			...
			top_k = [10, 10, 3] #  fix each neighor type size  固定每个邻接类型的大小
			for i in range(self.args.A_n):
				for j in range(3):
	...
	def sample_het_walk_triple(self):
		...
		P_n = self.args.P_n
		V_n = self.args.V_n
		triple_sample_p = self.triple_sample_p  # use sampling to avoid memory explosion 使用采样来避免内存爆炸
					...
					if centerNode[0] == 'a':
						for k in range(j - window, j + window + 1):
							if k and k < walk_L and k != j:
								...
								neighNode = path[k]
								# print('ne:',neighNode)
								# print('tr:',triple_sample_p)
								if neighNode[0] == 'a' and random.random() < triple_sample_p[0]:  #  random.random()生成0~1之间的浮点数 triple_sample_p为采样率
									...
									# 随机负采样获得与噪声分布采样相似的性能
									triple = [int(centerNode[1:]), int(neighNode[1:]), int(negNode)]  # a节点的邻居节点中a类型节点，negNode随机生成
									# print('trip:',triple)
									triple_list[0].append(triple)
									# print('trip:',triple_list)
